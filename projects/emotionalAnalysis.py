...
def cal_emotion_prob(filePath,landmarkName):
    ...
    totalProb=goodProbsAvg*(goodOpinionCount/totalOpinionCount)+\
              normalProbsAvg*(normalOpinionCount/totalOpinionCount)+badProbsAvg*(badOpinionCount/totalOpinionCount)
    return totalProb
...
if __name__ == "__main__":
    ...
    for i in locations:
        try:
            ...
            print(e)
            pass
    with open(filePath+"/probs.json", "w", encoding='utf-8') as fp:
        fp.write(json.dumps(locationProbs,indent=4,ensure_ascii=False))
