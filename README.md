...
打开`/sources/HomePage/index.html`查看首页。

打开`/sources/locations-page/*.html`查看省份页。

打开`/sources/scene-intro/*/*/*.html`查看景点页。

由于本地页面的链接跳转按照服务器中的网页存放方式链接，与GitHub仓库存放目录不同，因此在本地是无法完成各页面之间的跳转。
